...
I will represent my TaskQueue using a Sequence<Task>, where Task is a simple record containing a description and a priority.A Sequence<Task> keeps tasks ordered, allows efficient insertion by position, and matches the behavior required by the kernel methods (addTask, removeTask, isEmpty).
